...
// FILE: js/news-meteo.js (VERSIONE 55.0 - SELF-CONTAINED STYLES & ZERO-CACHE FAIL)
// ============================================================================
...
const NewsMeteoEngine = (function() {
  ...
  // 🔒 INIEZIONE CSS BLINDATA (Neutralizza i blocchi della cache di core.css)
  function injectComponentStyles() {
    if (document.getElementById("news-meteo-injected-styles")) return;
    const style = document.createElement("style");
    style.id = "news-meteo-injected-styles";
    style.textContent = `
      #news-meteo-mount {
        width: 100% !important;
      ...
      .news-meteo-mini-bar:hover {
        border-color: rgba(56, 189, 248, 0.45) !important;
      }
      .meteo-capsule-anchor {
        display: flex !important;
        align-items: center !important;
        gap: 4px !important;
      ...
      .meteo-capsule-anchor:hover {
        background: rgba(56, 189, 248, 0.15) !important;
        border-color: #38BDF8 !important;
      }
      .meteo-capsule-icon { font-size: 16px !important; line-height: 1 !important; }
      .meteo-capsule-temps {
        display: flex !important;
        align-items: center !important;
        gap: 2px !important;
      ...
      .meteo-temp-max { color: #FBBF24 !important; }
      .meteo-temp-divider { color: #475569 !important; font-weight: normal !important; }
      .meteo-temp-min { color: #38BDF8 !important; }
      .meteo-capsule-city {
      ...
      @media (min-width: 380px) {
        .meteo-capsule-city { display: inline-block !important; }
      }
      .news-ticker-window {
        flex: 1 !important;
      ...
      .news-ticker-track {
        ...
        animation: newsTickerScroll 38s linear infinite !important;
      }
      .news-ticker-window:hover .news-ticker-track,
      .news-ticker-window:active .news-ticker-track {
  ...
  async function fetchRemoteData(forceRefresh = false) {
    try {
      ...
      console.warn("[NewsMeteo] Fallback su cache locale:", err);
    }
  }
    ...
    modal.innerHTML = `
      <div class="modal-box p-4 bg-[#070A12] border border-sky-400/40 rounded-2xl max-w-sm sm:max-w-md space-y-3 relative shadow-2xl">
        ...
        <div id="tab-content-meteo" class="${activeModalTab === 'meteo' ? '' : 'hidden'} space-y-2.5">
          <div class="p-3 rounded-xl bg-gradient-to-br from-sky-950/40 to-slate-900 border border-sky-400/30 flex items-center justify-between">
            <div class="flex items-center gap-2.5">
              <span class="text-3xl" id="meteo-today-icon">☁️</span>
              <div>
          ...
          <div class="space-y-1">
            ...
            <div class="space-y-1 max-h-44 overflow-y-auto pr-1" id="meteo-forecast-list"></div>
          </div>
        </div>
        ...
        <div id="tab-content-news" class="${activeModalTab === 'news' ? '' : 'hidden'} space-y-2">
          ...
          <div class="space-y-1.5 max-h-56 overflow-y-auto pr-1" id="news-articles-list"></div>
        </div>
      </div>
    `;
  }
  ...
  function renderModalContent() {
    ...
    s("meteo-today-icon", oggi.icona || "☁️");
    ...
    if (fList) {
      ...
      if (prev.length === 0) {
        fList.innerHTML = `<div class="p-2 text-center text-xs text-slate-500 font-mono">Previsioni in aggiornamento...</div>`;
      } else {
